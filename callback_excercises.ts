...
const map = (array: number[], callback: (num: number) => number): number[] => {
  const result: number[] = [];
...
function forEach(array: string[], callback:(char: string)=> void ): void  {
    for(let i = 0; i < array.length; i++){
        callback(array[i]!)
    }
}
...
// 
// DONE ✅ ________________________________________________________________________________________________
// Challenge 15
/* 
...
*/
function rating<T>(
  callbacks: Array<(value: T) => boolean>,
  value: T
): number {
  let trueCount = 0;
  ...
  return (trueCount / callbacks.length) * 100;
}
...
function highestFunc(
  funcs: Record<string, (value: number) => number>,
  subject: number
): string {
  let highestKey = "";
  ...
  for (const key in funcs) {
    const result = funcs[key]!(subject);
